...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, hidden_dim in enumerate(hidden_dims):
            layer = str(i + 1)
            self.params['W' + layer] = weight_scale * np.random.randn(pre_dim, hidden_dim)
            self.params['b' + layer] = np.zeros(hidden_dim)
            pre_dim = hidden_dim
        ...
        if self.normalization:
            for i in range(1, self.num_layers):
                layer = str(i)
                self.params['gamma' + layer] = np.ones(hidden_dims[i - 1])
                self.params['beta' + layer] = np.zeros(hidden_dims[i - 1])
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization is None:
            for i in range(self.num_layers - 1):
                cache = []
                layer_input, cache1 = affine_relu_forward(layer_input, 
                    self.params['W%s' % (i + 1)], self.params['b%s' % (i + 1)])
                cache.append(cache1)
                if self.use_dropout:
                    layer_input, cache2 = dropout_forward(layer_input, self.dropout_param)
                    cache.append(cache2)
                caches.append(cache)
        else:
            if self.normalization == 'batchnorm':
